...
function processLibrary() {
  ...
  // Emit a denylist of every reference-only workId. The public by-work route
  // reads this and refuses to serve a body for any listed work — defense in
  // depth, since the route otherwise treats an absent `contentStatus` as
  // full-text and would happily serve a body file that gets re-introduced
  // (which is exactly how the audited works leaked the first time).
  const referenceOnlyIds = catalog.works
    .filter((w) => w.contentStatus === "reference-only")
    .map((w) => w.id)
    .sort();
  writeJson(path.join(LIBRARY_DIR, "reference-only.json"), {
    note: "Generated by scripts/migrate/license-cleanup/run.ts. Work ids whose body prose is not redistributed in-app; the by-work API route returns 404 for these.",
    workIds: referenceOnlyIds,
  });
